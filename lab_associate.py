...
'''
    Prints out monitored equipment ordered by monitor with filters.
'''
...
'''
signup for equipment monitoring
'''
def signup_to_monitor(cursor,user):
    ...
    query = ("""select as_t.equ_id as 'ID', equ_name as 'Equipment' 
    ...
    if signup == 'y' or signup == 'Y':
        ...
        if equ in dict(result_set):
            ...
            if cursor.rowcount > 0:
                ...
                query = """select as_t.equ_id as 'ID' , equ_name as 'Equipment' , as_t.monitor_id as 'Monitor' 
                from assigned_to as as_t join equipment as equ
                on as_T.equ_id = equ.equ_id
                where monitor_id = '%s' 
...
'''
Prints out maintainted equipment from user
'''
def view_maintained_equipment(cursor,user):
    query = """select equ_id as 'ID', equ_name as 'Equipment' 
                from equipment 
		        where maintained_by = '%s'; """%(user)
    utils.query_executor(cursor,query)
    ...
    if len(result_set)==0:
        ...
        print("Here are your allocated equipment for maintenance:\n")  
        utils.display_json(json_result)
    
'''
    Prints out currently monitored equipment with filter.
'''
...
'''
    Prints out past monitored equipment with filter.
'''
...
def la_handler(cursor,user):
    ...
    options_array = [   ("View Equipment By Monitor",view_equipment_by_monitor),
                    ...
                    ("View Maintained Equipment",view_maintained_equipment),
    ...
    while(choice!=len(options_array)):
        choice = utils.getChoice(options_string)
